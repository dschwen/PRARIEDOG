...
EventInserterBase::EventInserterBase(const InputParameters & parameters) :
...
void
EventInserterBase::initialSetup()
{
...
void
EventInserterBase::execute()
{
  // print out events for debugging
  if (_verbose)
  {
    _console << "event lists at beginning of EventInserterBase::execute() for time step " << _t_step << " and time " << _t << std::endl;
    printEventLists();
  }
  ...
  if (_verbose)
  {
    _console << "event lists at end of EventInserterBase::execute() for time step " << _t_step << " and time " << _t << std::endl;
    printEventLists();
  }
...
Real
EventInserterBase::getNewEventInterval()
{
  Real mean = computeMean();  // get mean event time, implemented in derived classes

  if (_use_random_timing)
  {
    if (_distribution == "uniform")
      return _random.rand(0)*mean*2.0;
    else // exponential distribution
      return -std::log(_random.rand(0))*mean;
  }
...
bool
EventInserterBase::isEventActive(const Real t1, const Real t2) const
{
...
Point
EventInserterBase::getActiveEventPoint(const Real t1, const Real t2) const
{
  for (unsigned int i=0; i<_global_event_list.size(); i++)
    if ((_global_event_list[i].first >= t1 - _time_tol) && (_global_event_list[i].first < t2))
      return _global_event_list[i].second;

  mooseError("EventInserterBase::getActiveEvent: Active event point requested but no active event found. Check with `isEventActive(t1, t2)` first.");
}

Real
EventInserterBase::getNextEventTime() const
{
...
Real
EventInserterBase::getMaxEventTime() const
{
...
void
EventInserterBase::printEventLists() const
{
